...
function parse(input, options) {
  ...
  if (input.indexOf("NOTE Confidence") > 0) {
    input = input.replace(/NOTE/g, "\n NOTE");
    input = input.replace(/\r\n/g, "\n");
    input = input.replace(/\r/g, "\n");
    parts = input.split("\n\n\n");
  } else {
...
function parseCue(cue, i, strict) {
  ...
  if (
    ...
    lines[0].trim().indexOf('NOTE Confidence') == -1
  ) {
    return null;
  }
  ...
  if (
    ...
    !(
      lines[0].includes('-->') ||
      lines[1].includes('-->') ||
      lines[0].includes('NOTE Confidence')
    )
  ) {
  ...
  if (cue.trim().indexOf('NOTE Confidence') != -1) {
    const confidenceArray = lines.shift().split('NOTE Confidence: ');
    confidence = Math.round(confidenceArray[1] * 100) + '%';
  }
  ...
  return { identifier, start, end, text, styles, confidence };
}
