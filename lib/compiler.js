...
function compile(input) {
  ...
  if (input.meta && typeof input.meta !== 'object') {
    throw new CompilerError('Input metadata must be an object');
  }

  let output = 'WEBVTT\n';

  if (input.meta) {
    Object.keys(input.meta).forEach(key => output += `${key}:${input.meta[key]}\n`);
  }
...
function compileCue(cue) {
  ...
  if (cue.notes && !Array.isArray(cue.notes)) {
    throw new CompilerError(`Cue malformed: notes must be an array.
    ${JSON.stringify(cue)}`);
  }

  let output = '';

  (cue.notes || []).forEach(note => output += `NOTE ${note}\n\n`);
